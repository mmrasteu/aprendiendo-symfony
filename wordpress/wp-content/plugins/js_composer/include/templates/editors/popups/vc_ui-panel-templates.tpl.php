			...
			<div class="vc_ui-panel-content vc_properties-list"
				...
				<div class="vc_panel-tabs">
					...
					foreach ( $categories_data as $key => $category ) :
						...
						/* UPPER additon - library sidebar*/
						if( function_exists('upper_vc_library_cat_list') && esc_attr( $category['category'] ) == 'default_templates') {
							echo '<div class="library_categories">
								<ul>';
									$upper_library_cats = upper_vc_library_cat_list();
									foreach($upper_library_cats as $cat_name => $cat_sort_text) {
