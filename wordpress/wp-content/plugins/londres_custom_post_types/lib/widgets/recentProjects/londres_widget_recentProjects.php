...
class Londres_Projects_Widget extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'projects_widget', 'description' => esc_html__('Show your Projects on your site.','londres'));
	...
	function form($instance) {
       ...
       <!-- NEW -->
       <p>
	        <label>&#8212; <?php esc_html_e('Cube Portfolio','londres'); ?> &#8212;<br>
	        <?php
				...
				if (!empty($cbps)){
					...
					?>
					<p><?php esc_html_e("There are no cubeportolio instances.", "londres"); ?></p>
					<?php
				}
	        ?>
	        </label>
	   ...
	   <p class="projects_hidearrows_select"><label for="<?php echo esc_attr($this->get_field_id('hidearrows')); ?>">&#8212; <?php esc_html_e('Hide Navigation Arrows','londres'); ?> &nbsp;<input id="<?php echo esc_attr($this->get_field_id('hidearrows')); ?>" name="<?php echo esc_attr($this->get_field_name('hidearrows')); ?>" type="checkbox" value="hidearrows" <?php if($hidearrows == "hidearrows") echo 'checked'; ?> /></label></p>
		...
		<p class="projects_hidenav_select"><label for="<?php echo esc_attr($this->get_field_id('hidenav')); ?>">&#8212; <?php esc_html_e('Hide Navigation','londres'); ?> &nbsp;<input id="<?php echo esc_attr($this->get_field_id('hidenav')); ?>" name="<?php echo esc_attr($this->get_field_name('hidenav')); ?>" type="checkbox" value="hidenav" <?php if($hidenav == "hidenav") echo 'checked'; ?> /></label></p>
		...
		$londres_admin_inline_script = '
			jQuery(document).ready(function(){
				"use strict";
				jQuery("#'.esc_js($this->get_field_id('cubeid')).'").on("change",function(){ jQuery(this).find("option[selected]").removeAttr("selected"); });
			});
		';
	...
	function widget($args, $instance) {
		...
		?>
		<div class="des_cubeportfolio_widget_helper londres_helper_div cubeid-<?php echo esc_attr($cubeid); ?>"><?php
		...
		echo '<div class="widget des_cubeportfolio_widget">';
			...
			$getCubeCSS = londres_get_string_between($getCube, "<style type='text/css'>", "</style>" );
					    ...
					    if (timeout === false) {
					        ...
					        setTimeout(function(){
								...
								jQuery(".ult-carousel-'.esc_js($uid).' .cbp-item").each(function(){ 
									...
									jQuery(this).find("a.cbp-singlePage").on("click",function(){
								        jQuery(".des_cubeportfolio_widget_helper.cubeid-'.esc_js($cubeid).' .cbp-item").eq(jQuery(this).closest(".ult-item-wrap").data("slick-index")).find("a.cbp-singlePage").trigger("click"); 
							        });
							        jQuery(this).find("a.cbp-singlePageInline").on("click",function(){
								        jQuery(".des_cubeportfolio_widget_helper.cubeid-'.esc_js($cubeid).' .cbp-item").eq(jQuery(this).closest(".ult-item-wrap").data("slick-index")).find(".cbp-singlePageInline").trigger("click"); 
							        });
								});
					        }, delta);
					    }
		...
		londres_set_custom_inline_css($londres_inline_css);
				
		echo ob_get_clean();
	}
}
register_widget('Londres_Projects_Widget');
?>