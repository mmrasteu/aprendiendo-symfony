...
*/
function londres_pagenavi_textdomain() {
	load_plugin_textdomain('wp-pagenavi', false, 'wp-pagenavi');
}

function londres_wp_pagenavi($before = '', $after = '') {
	global $wpdb, $wp_query, $londres_the_query;
	...
	if (!is_single()) {
		...
		if (!empty($londres_the_query)){
			$wp_query = $londres_the_query;
		}
		...
		if (empty($londres_the_query)) $max_page = ceil($totalPosts / $posts_per_page);
		...
		$larger_start_page_start = (londres_n_round($start_page, 10) + $larger_page_multiple) - $larger_per_page;
		$larger_start_page_end = londres_n_round($start_page, 10) + $larger_page_multiple;
		$larger_end_page_start = londres_n_round($end_page, 10) + $larger_page_multiple;
		$larger_end_page_end = londres_n_round($end_page, 10) + ($larger_per_page);
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			$pages_text = str_replace("_CURRENT_PAGE_", $paged, esc_html__('Page _CURRENT_PAGE_ of _TOTAL_PAGES_','londres'));
			$pages_text = str_replace("_TOTAL_PAGES_", $max_page, $pages_text);
			$pages_text = esc_html__("Page ","londres") . $paged . esc_html__(" of ","londres") . $max_page;
			echo '<div class="des-pages">'."\n";
			
			switch(get_option("londres_blog_reading_type")){
			...
			switch(intval($pagenavi_options['style'])) {
				...
				case 2:
					...
					echo '<label class="gotopagedd" for="pagedd">'. esc_html__('Go to page','londres') .'</label>';
...
function londres_wp_pagenavi_dropdown() { 
	londres_wp_pagenavi(); 
}

### Function: Round To The Nearest Value
function londres_n_round($num, $tonearest) {
   return floor($num/$tonearest)*$tonearest;
}

### Function: Page Navigation Options
function londres_pagenavi_init() {
	londres_pagenavi_textdomain();
	// Add Options
	$pagenavi_options = array();
	$pagenavi_options['pages_text'] = esc_html__('Page _CURRENT_PAGE_ of _TOTAL_PAGES_','londres');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = esc_html__('&laquo;','londres');
	$pagenavi_options['last_text'] = esc_html__('&raquo;','londres');
	$pagenavi_options['next_text'] = esc_html__('&raquo;','londres');
	$pagenavi_options['prev_text'] = esc_html__('&laquo;','londres');
	$pagenavi_options['dotright_text'] = esc_html__('...','londres');
	$pagenavi_options['dotleft_text'] = esc_html__('...','londres');
