<?php
function londres_the_breadcrumb(){
	$delimiter = $delimiter1 = "";
    $main = sprintf(esc_html__("%s", 'londres'), get_option('londres_breadcrumbs_home_text'));
	if (function_exists('icl_t')){
		$main = sprintf(esc_html__("%s", 'londres'), icl_t( 'londres', 'Home', get_option('londres_breadcrumbs_home_text')));
	}
    $maxLength= 30;
    ...
    if (!is_front_page()) {         
        ...
        elseif ( is_search() ) {  
            echo "<span>".esc_html__('Search Results for "', 'londres') . get_search_query() . '"</span>';
        }       
        ...
        elseif ( is_author() ) {
            ...
            echo  "<span>".esc_html__('Author&#39;s Article(s) ', 'londres') . wp_kses_post($delimiter . $user_info->display_name)."</span>" ;
        }       
        elseif ( is_404() ) { }
        else {
           	global $wpdb;
