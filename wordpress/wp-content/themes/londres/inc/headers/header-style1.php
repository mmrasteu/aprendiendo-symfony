...
function londres_print_menu($ispagephp = true, $isfirstpage = false){
	...
	<header class="navbar navbar-default navbar-fixed-top <?php echo esc_attr($typeofheader); ?> <?php if (get_option('londres_fixed_menu') == 'off') echo " header_not_fixed"; else if (get_option('londres_header_hide_on_start') == "on" && !$ispagephp) echo " hide-on-start"; ?><?php if (get_option("londres_header_full_width") == "on") echo " header-full-width"; ?><?php if (get_option("londres_header_full_width") == "off") echo " header-with-container"; ?><?php if (get_option("londres_header_menu_itens_style") == "rounded") echo " menu-rounded"; ?><?php if (get_option("londres_header_menu_itens_style") == "simple") echo " menu-simple"; ?><?php if (get_option("londres_header_menu_itens_style") == "square") echo " menu-square"; ?><?php echo " ".esc_attr($londres_header_bgstyle_pre); ?>" data-rel="<?php echo esc_attr($londres_header_bgstyle_pre."|".$londres_header_bgstyle_after); ?>">
		...
		if (get_option("londres_info_above_menu") == "on"){
			...
			<div class="top-bar">
				<div class="top-bar-bg">
					<div class="<?php if (get_option("londres_header_full_width") == "off") echo "container"; ?> clearfix">
						<div class="slidedown">
								...
								if ( get_option("londres_telephone_menu") != "" || get_option("londres_email_menu") != "" || get_option("londres_address_menu") != "" || get_option("londres_text_field_menu") != "" ){
									...
									<ul class="phone-mail">
										...
										<?php if ( is_string(get_option("londres_text_field_menu")) && get_option("londres_text_field_menu") != "" ){ ?>
											<li class="text_field"><?php echo wp_kses_post(get_option("londres_text_field_menu")); ?></li>
										<?php } ?>
									</ul>
									<?php
								}
		...
		<div class="nav-container <?php if (get_option("londres_header_full_width") == "off") echo " container" ?> <?php if (class_exists( 'WooCommerce' ) && get_option("londres_woocommerce_cart") == "on") echo "nav-with-woocommerce-cart"; ?>">
			...
			<div class="londres_right_header_icons <?php
				if (class_exists( 'WooCommerce' ) && get_option("londres_woocommerce_cart") == "on") echo "with-woocommerce-cart";
			?>">
				...
				<?php londres_print_woocommerce_button();?>
			...
			<?php
				if (!$isfirstpage){
					...
					<div id="dl-menu" class="dl-menuwrapper">
						...
						<?php 
							if ($ispagephp){
								wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'dl-menu', 'walker' => new londres_walker_nav_menu_outsider_mobile, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','londres') ) );
							} 
							else {
								global $homes;
								$homes = 0;
								wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'dl-menu', 'walker' => new londres_walker_nav_menu_mobile, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','londres') ) );
							} 
						?>
					</div>
					<?php
			...
			if (get_option("londres_enable_search") == "on"){
				...
				<form autocomplete="off" role="search" method="get" class="search_input <?php echo esc_attr(get_option("londres_search_open_effect")); ?>" action="<?php echo esc_url(home_url("/")); ?>">
					...
					<div class="<?php if (get_option("londres_header_full_width") == "off") echo "container"; ?>">
						<input value="" name="s" class="search_input_value" type="text" placeholder="<?php
							if (function_exists('icl_t')){
								printf(esc_html__("%s","londres"), icl_t( 'londres', 'Type your search and hit enter...', get_option('londres_search_box_text')));
							} else {
								printf(esc_html__("%s","londres"), get_option("londres_search_box_text"));
							}
						?>" />
