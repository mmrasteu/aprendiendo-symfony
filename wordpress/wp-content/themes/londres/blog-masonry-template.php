...
get_header(); londres_print_menu();
	...
	if ($type != "without"){
		...
		<div class="fullwidth-container <?php if ($type == "pattern") echo "bg-pattern"; ?> <?php if ($pt_parallax) echo "parallax"; ?><?php if (($type == "image" || $type == "pattern") && get_option('londres_enable_grayscale') == 'on') echo " londres_grayscale "; ?>" <?php if ($pt_parallax) echo 'data-stellar-ratio="0.5"'; ?> 
	    	<?php
				...
				if (defined("LONDRES_IS_FIRST_PAGE")) $londres_output.= "margin-top:60px;";
				$londres_output .= "}";
					...
					<section class="page_content">
						<div class="container">
							<?php 
								...
								else londres_print_masonry_blog();
							?>
						</div>
					</section>
				</div>
	...
	function londres_print_masonry_blog(){
	    ...
	    if ($londres_the_query->have_posts()){ 
	    	...
	    	if (get_post_meta(get_the_ID(), 'posts_add_category_filter_value', true) == 'yes' && is_array($categories) && count($categories) > 2){
		    	...
		    	<ul class="posts_category_filter">
			    	<li class="showall selected"><?php esc_html_e('All','londres'); if (get_post_meta(get_the_ID(), 'posts_add_category_filter_counter_value', true) == 'yes'){ ?>
							    		...
							    		case "image":
											...
											?>
												...
												<div class="padding-box-masonry">
													...
													londres_set_custom_inline_css($londres_output);
													if (isset($wp_query->queried_object->ID) && get_post_meta( $wp_query->queried_object->ID , 'upper_single_display_metas_value' , true ) == "yes"){
														?>
														<div class="metas">
