...
if ( isset($_GET['page']) && $_GET['page'] == LONDRES_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'londres_init_options_functionality');  
	add_action('init', 'londres_set_options'); 
}

if ( isset($_GET['page']) && $_GET['page'] == LONDRES_STYLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'londres_init_style_options_functionality');  
	add_action('init', 'londres_set_style_options'); 
}
 ...
 */
function londres_init_style_options_functionality(){
	global $londres_style_options;

	$londres_style_options=array();

	//get all the categories
	$categories=get_categories('hide_empty=0');
	$londres_categories=array();
	for($i=0; $i<sizeof($categories); $i++){
		$londres_categories[]=array('id'=>$categories[$i]->cat_ID, 'name'=>$categories[$i]->cat_name);
	}

	//load the files that contain the options
	$londres_style_options_files=array('style-general', 'style-website-loading', 'style-header-top-bar', 'style-logotype', 'style-menu', 'style-sliding-panel', 'style-search', 'style-pagetitle', 'style-footer', 'style-typography' , 'style-import-export');
	foreach($londres_style_options_files as $file){
		require_once(LONDRES_OPTIONS_PATH.$file.'.php');
	}
}
...
function londres_set_style_options(){
	...
	if ( $_GET['page'] == LONDRES_STYLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['londres-theme-style-options'],'londres-theme-update-style-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("londres_style_first_save")){
						update_option("londres_style_first_save",'true');
					}
					...
					$doc->save($filename);		
					header("Location: themes.php?page=".LONDRES_STYLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 * to populate the global options array.
 */
function londres_init_options_functionality(){
	global $londres_options;

	$londres_options=array();
	...
	//load the files that contain the options
	$londres_options_files = array('general', 'header-menu', 'widgetsarea', 'sliders', 'translation', 'social', 'data');
	if (defined('LONDRES_PLG_ACTIVE') === true) $londres_options_files = array('general', 'header-menu', 'widgetsarea', 'sliders', 'newsletter', 'translation', 'social', 'customcss' , 'data');
	
	foreach($londres_options_files as $file){
		require_once(LONDRES_OPTIONS_PATH.$file.'.php');
	}
}
...
function londres_set_options(){
	...
	//insert the default values if the fields are empty
	foreach ($londres_options as $value) {
		if(isset($value['id'])){
			if(get_option($value['id'])=='' && isset($value['std']) && !in_array($value['type'], $nonsavable_types)){
				update_option( $value['id'], $value['std']);
			}
	...
	//save the field's values if the Save action is present
	if ( $_GET['page'] == LONDRES_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['londres-theme-options'],'londres-theme-update-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("londres_first_save")){
						update_option("londres_first_save",'true');
					}
					...
					foreach ($londres_options as $value) {
						...
						if (isset($value['id'])){
							if( isset( $_REQUEST[ $value['id'] ] ) && !in_array($value['type'], $nonsavable_types)) {
								...
								if ($value['id'] === "londres_portfolio_permalink"){
									$oldval = get_option("londres_portfolio_permalink");
								...
								if ($value['id'] == 'londres_instagram_limit'){
									$oldval = get_option('londres_instagram_limit');
									$newval = $_REQUEST[$value['id']];
									if ($oldval !== $newval){
										delete_option('_transient_timeout_londres_insta_transient_scraper');
										delete_option('_transient_londres_insta_transient_scraper');
										delete_option('_transient_timeout_londres_insta_transient_user_items');
										delete_option('_transient_londres_insta_transient_user_items');
									}
								}
					...
					$doc->appendChild($root);
					$doc->save($filename);		
					header("Location: themes.php?page=".LONDRES_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 */
function londres_theme_admin() {
	global $londres_options,$londres_options_manager;

	$londres_options_manager=new LondresOptionsManager("Londres", LONDRES_IMAGES_URL, LONDRES_UTILS_URL, LONDRES_UPLOADS_URL, LONDRES_VERSION);
	$londres_options_manager->set_options($londres_options);

	if ( isset($_REQUEST['saved'] )) {
		$londres_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$londres_options_manager->print_reset_message();
	}

	$londres_options_manager->print_heading("");
	$londres_options_manager->print_options();
	$londres_options_manager->print_footer();
}
 ...
 */
function londres_theme_style_options_admin() {
	global $londres_style_options, $londres_style_options_manager;

	$londres_style_options_manager=new LondresStyleOptionsManager("Londres", LONDRES_IMAGES_URL, LONDRES_UTILS_URL, LONDRES_UPLOADS_URL, LONDRES_VERSION);
	$londres_style_options_manager->set_options($londres_style_options);

	if ( isset($_REQUEST['saved'] )) {
		$londres_style_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$londres_style_options_manager->print_reset_message();
	}

	$londres_style_options_manager->print_heading("");
	$londres_style_options_manager->print_options();
	$londres_style_options_manager->print_footer();
}
 ...
 */
function londres_theme_demos_admin() {
	global $londres_demos_manager;

	$londres_demos_manager=new LondresDemosManager("Londres", LONDRES_IMAGES_URL, LONDRES_UTILS_URL, LONDRES_UPLOADS_URL, LONDRES_VERSION);

	$londres_demos_manager->print_heading("");
	$londres_demos_manager->print_options();
	$londres_demos_manager->print_footer();
}
 ...
 */
function londres_add_options($option_arr){
	global $londres_options;

	foreach($option_arr as $option){
		$londres_options[]=$option;
	}
}

function londres_add_style_options($option_arr){
	global $londres_style_options;

	foreach($option_arr as $option){
		$londres_style_options[]=$option;
	}
}
 ...
 */
function londres_get_opt($option){
	return stripslashes(get_option("londres".$option));
}


function londres_get_custom_sidebars(){
	$res=array();
	$sides=get_option('londres_sidebar_name_names');
	if($sides){
		$res=explode(LONDRES_SEPARATOR, $sides);
		array_pop($res);
	}
	return $res;
}
 ...
 */
function londres_add_custom_options($opt_array){
	$new_londres_options=array();
	...
	foreach($opt_array as $option){
		if($option['type']=='multiple_custom'){
			...
			$saved_array=explode(LONDRES_SEPARATOR, $saved_values);
			if(sizeof($saved_array)>1){
				array_pop($saved_array);
				foreach($saved_array as $custom_name){
					$id=londres_convert_to_class($custom_name);
					$custom_option=array(
					"id"=>$id,
					...
					array_push($new_londres_options, $custom_option);
				}
			}
				
		} else{
			//this is just a normal option, just append it into the new array
			array_push($new_londres_options, $option);
		}
	}

	return $new_londres_options;
}

function londres_fonts_array_builder(){
	$defaultfonts = array();
...
function londres_portfolio_types(){
...
function londres_get_proj(){
	
	$londres_proj2=array(array('id'=>'default', 'name'=>'Default'));
	if (function_exists('londres_get_projects')){
		$londres_proj1 = londres_get_projects();
		foreach($londres_proj1 as $dp){
			$londres_proj2[]=array("name"=>$dp['p_title'], "id"=>$dp['p_id']);
		}
	}

	return $londres_proj2;
}


function londres_get_all_google_fonts(){
	$londres_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
