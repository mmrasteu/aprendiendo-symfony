 ...
 *
 */
Class LondresCustomPageManager{
	...
	function build_page(){
		...
		$this->templater=new LondresTemplater();
		...
		echo wp_kses_post('<input type="hidden" value="'.esc_attr($this->taxonomy).'" id="taxnonomy_id" />
			  ...
			  <input type="hidden" value="'.esc_attr(wp_create_nonce(LONDRES_NONCE)).'" id="londres_nonce" />
		      </div>');
	}
	...
	function _print_items($category){
		...
		$ordered_posts=londres_get_ordered_post_list($posts, $category,$this->custom_page->post_type);
